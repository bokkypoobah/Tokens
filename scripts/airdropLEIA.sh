...
for (var i = 0; i < target.length; i++) {
  ...
  if (balance.lt(airdropAmount)) {
    ...
    // var tx = leia.transfer(target[i], amount, {from: eth.accounts[26], gas: 100000, gasPrice: web3.toWei(1, "gwei")});
    // console.log("  - tx " + tx);
  }
}
